...
.data ; Phần khai báo dữ liệu
    ...
    is_increasing dword ? ; Biến lưu trạng thái kiểm tra số nguyên dương n có các chữ số tăng dần hay không
...
main proc ; Bắt đầu quá trình thực thi của chương trình
    ...
    mov is_increasing, 1 ; Giả sử ban đầu rằng tất cả các chữ số tăng dần
    ...
    while_loop: ; Nhãn của vòng lặp
        ...
        jg set_not_increasing ; Nếu chữ số hiện tại lớn hơn chữ số tiếp theo, đặt biến is_increasing thành 0 và thoát khỏi vòng lặp

        jmp while_loop ; Lặp lại vòng lặp

    set_not_increasing: ; Nhãn để đặt biến is_increasing thành 0
        mov is_increasing, 0 ; Đặt biến is_increasing thành 0
        jmp end_while ; Kết thúc vòng lặp

    end_while: ; Nhãn kết thúc vòng lặp

    ; In kết quả ra màn hình
    invoke StdOut, addr is_increasing ; In ra trạng thái kiểm tra số nguyên dương n có các chữ số tăng dần hay không
