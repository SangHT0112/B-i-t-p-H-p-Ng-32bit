...
.data ; Phần khai báo dữ liệu
    ...
    is_all_even dword ? ; Biến lưu trạng thái kiểm tra số nguyên dương n có toàn chữ số chẵn hay không
...
main proc ; Bắt đầu quá trình thực thi của chương trình
    ...
    mov is_all_even, 1 ; Giả sử ban đầu rằng tất cả các chữ số đều là chẵn
    ...
    while_loop: ; Nhãn của vòng lặp
        ...
        test edx, 1 ; Kiểm tra xem chữ số dư có phải là số chẵn không
        jnz set_not_all_even ; Nếu là số lẻ, đặt biến is_all_even thành 0 và thoát khỏi vòng lặp

        jmp while_loop ; Lặp lại vòng lặp

    set_not_all_even: ; Nhãn để đặt biến is_all_even thành 0
        mov is_all_even, 0 ; Đặt biến is_all_even thành 0
        jmp end_while ; Kết thúc vòng lặp

    end_while: ; Nhãn kết thúc vòng lặp

    ; In kết quả ra màn hình
    invoke StdOut, addr is_all_even ; In ra trạng thái kiểm tra số nguyên dương n có toàn chữ số chẵn hay không
